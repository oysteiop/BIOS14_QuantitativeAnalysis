...
In our last section on (simple) linear models, we will consider the case there the predictor variables include both continuous variables and factors. Such analyses are referred to as analyses of covariance.
...
### DATA EXERCISE: Analysis of covariance
Choose one of your own datasets, or either the `bird_allometry` dataset or the `blossoms` dataset. Formulate one or more hypotheses that can be tested in an ANCOVA. Perform the analysis, interpret the results, produce neat figures, write methods and results.
...
- Eigendecomposition and PCA. Matrix comparison?
