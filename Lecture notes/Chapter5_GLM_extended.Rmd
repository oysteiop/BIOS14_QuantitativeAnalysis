...
```
In the Results, we could then write "The abundance of *Eulaema nigrita* varied substantially along the forest-cover gradient (Fig. 1). Assuming average annual rainfall, predicted bee abundance ranged from 105 bees for a site in a completely unforested landscape to 28 bees in a completely forested landscape."
...
```{r, fig.height=4, fig.width=4, fig.cap="Variation in Eulaema bee abundance along a forest-cover gradient, where 0 represents an entirely unforested landscape, and 1 a completely forested landscape. Regression lines illustrate the effect when mean annual precipitation is at its mean, one SD below the mean, and one SD above the mean"}
