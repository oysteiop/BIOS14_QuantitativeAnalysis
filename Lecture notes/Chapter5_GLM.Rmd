...
One previously popular but now not recommended (see Warton & Hui 2011 Ecology) transformation is the so-called arcsine square-root transformation,
...
## Logistic regression
Given the properties of the logit and similar link functions outlined above, it follows that logistic regression (GLM with binomial errors) is well suited for analysing binary data (or proportions).

Below, we simulate data by first formulating a linear predictor $\eta$, then transforming the predicted values into probabilities (through the inverse-logit transformation), and finally binarizing the data by sampling from the binomial distribution. The last step adds additional realistic uncertainty by accounting for the stochasticity of the observation process (alhough the underlying probabilities range continuously from 0 to 1, we only "see" ones and zeros.
...
To fit the generalized linear model, we use the `glm` function and specify the error family (binomial) and link function (here *logit*).
...
The summary table includes, as always, a lot of information. The first thing to be aware of is that when we are fitting a GLM, we obtain the parameter estimates on the link scale (here logit). Note that the parameter estimates are not too far from those we used to define the linear predictor $\eta$ when we simulated the data. These values are meaningful as such, and if the predictor variable has units of *mm*, the slopes have units of *log odds* $mm^{-1}$.

To interpret the results biologically and to represent them in graphs, it can be useful to back-transform the predicted values to the probability scale. For example, we can ask how much the probability changes for a standard deviation increase in the predictor variable (although note that this is no longer a linear transform, so the consequences of increasing and decreasing the predictor by one standard deviation may be different).

Recall from the previous section that a log odds of 0 corresponds to a probability of 0.5  ($log(\frac{0.5}{1-0.5})=log(1)=0$). If we solve the model equation (the linear predictor) for 0, we can thus obtain the predictor value corresponding to a probability of 0.5, which is often a relevant benchmark. A classic example comes from toxicology, where it is often of interest to compute a "lethal dose 50 ($LD_{50}$), the dose of a substance associated with a 50% probability of death.
...
The GLM summary table does not provide an $r^2$ value, because the normal $r^2$ does not work for logistic regression. There are however several 'Pseudo-$r^2$' available, typically based on comparing the likelihood of the model to that of a null model (a similar model but with only an intercept). The `MuMIn` package provides some such measures. The package returns both a marginal ($r^2_{M}$) and a conditional $r^2$ ($r^2_{C}$). For now, we will focus only on the former, which is akin to the usual $r^2$ from linear models. We will return to the ($r^2_{C}$) when discussing mixed models. Because it is not entirely obvious how to compute the residual variance in these models (and hence the $r^2$), the package provides several options. Perhaps the most flexible of these is estimating the variance through the delta method.
...
The coefficient of discrimination is defined as $D=\bar{\hat{\pi_1}}-\bar{\hat{\pi_0}}$, and is computed by comparing the predicted probability for those data points that are successes or positives (1s; $\bar{\hat{\pi_1}}$) to the predicted probability for those data points that are failures or negatives (0s; $\bar{\hat{\pi_1}}$).
...
Some final notes on fitting binomial GLMs. There are three ways to formulate these models in `R`. In the example above, the data were 0's and 1's, and we could specify the model simply as 
...
After different duration of after-ripening, the seeds were sown on moist soil and their germination success (proportion of seeds germinated) recorded. The seeds came from four different populations, and were weighed (in *mg*) prior to sowing.
