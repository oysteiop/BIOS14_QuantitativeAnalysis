...
The GLM summary table does not provide an $r^2$ value, because the normal $r^2$ does not work for logistic regression. There are however several 'Pseudo-$r^2$' available, typically based on comparing the likelihood of the model to that of a null model (a similar model but with only an intercept). The `MuMIn` package provides one such measure. The `r.squaredGLMM` function provides several alternative $r^2$ measures. The reason is that, unlike in a Gaussian model where the residual variance is well defined, the residual variance in a binomial GLM has to be approximated one way or the other. The two alternatives used by the function is the theoretical variance of the binomial distribution (as discussed above), and an approximation based on the delta method (a statistical method/theorem allowing the approximation of a function´s probability distribution). The values will differ, and which to choose is often a matter of taste.
...
lines(xvals, invlogit(y_hat3), lty=3)
...
```{r}
library(MASS)
m = glm(Eulaema_nigrita~MAP+Pseason, fam="poisson", data=dat)
summary(m)

newMAP = seq(min(dat$MAP), max(dat$MAP), length.out=200)
newPseason = rep(mean(dat$Pseason), 200)

predvals = predict(m, newdata=list(MAP=newMAP, Pseason=newPseason), type="response")

plot(dat$MAP, dat$Eulaema_nigrita)
lines(newMAP, predvals, lwd=2)

####
m = glm.nb(Eulaema_nigrita~MAP+Pseason, data=dat)
summary(m)
