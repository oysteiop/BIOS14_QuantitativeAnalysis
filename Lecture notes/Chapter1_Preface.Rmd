...
Like most biostatistics texts this is a reference in which to look up specific topics, rather than a book to be read chapter by chapter. Personally I have also found Michael Crawley's brick "The R Book" useful in the past, it's as much a statistics text as an `R` text.
...
plot(sdvals, out, las=1, ylim=c(17.5, 20.5),
     xlab="Standard deviation", ylab="Geometric mean")
abline(h=20)
text(7.5, 20.22, "Arithmetic mean")
```
...
To understand the lines of code below, recall that you can access the help file for any `R` function by typing `?function`, here e.g. `?sample` to see which arguments can be passed to the `sample` function, and what their default values are. When you are new to `R`, you may have to look up the arguments for every function, but soon you will learn the basics of many functions and it will get easier.

```{r}
out = NULL
for(i in 1:1000){
  sample1 = sample(x, replace=TRUE)
  out[i] = mean(sample1)
}
```
