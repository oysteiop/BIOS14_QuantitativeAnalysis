...
1. Avoid empty cells, put `NA` instead. There are ways to make `R` insert `NA`s in empty cells, but to be safe it is better to add them already during data entry.
...
### Writing functions
`R`-functions take arguments, perform operations and, often, return some results. Base `R` does not come with a built-in function to compute standard errors, so let's make out own.
